...
> 创建：2026-09-22 ｜ 当前版本：v2.19 ｜ 状态：🔨开发中
> 页面/模块：`src/FakeSqlCompare` WPF；真实连接 + DacFx 对比 + 脚本/部署
> 状态说明：互换源/目标只留在连接页，对比页已去掉
...
| 差异工作台 | 表一行；定义脚本含 **CREATE TABLE（主键/外键/默认）+ NOCHECK + CREATE INDEX**；触发器仍单独。互换只在连接页 |
...
| UI | 连接下拉显示摘要；**载入**；类型芯片为表/视图/过程/函数；对象列表与并排定义之间可拖高度；自定义 `Assets/app.ico`；**互换仅连接页** |
...
| 版本 | 时间 | 变更内容 |
|---|---|---|
| v2.19 | 2026-09-23 11:12 | 对比页去掉「互换源/目标」，只在连接页中间 ⇄ 操作 |

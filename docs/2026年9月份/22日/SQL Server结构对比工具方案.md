...
> 创建：2026-09-22 ｜ 当前版本：v2.8 ｜ 状态：🔨开发中
> 页面/模块：`src/FakeSqlCompare` WPF；真实连接 + DacFx 对比 + 脚本/部署
> 状态说明：连接可另存别名；源/目标可互换
...
|---|---|---|
| P0 | 表（列、类型、可空、默认、标识列） | ✅ |
| P0 | 主键 / 唯一约束 / 默认约束 / Check | ✅ |
| P0 | 索引（含筛选索引、包含列） | ✅ |
| P0 | 外键 | ✅ |
| P0 | 视图、存储过程、函数（标量/内联/多语句） | ✅ |
| P0 | DML 触发器 | ✅ |
| P1 | Schema、同义词、序列、用户定义类型 | 第二期 |
| P2 | 分区函数/方案、安全对象、Extended Properties | 第三期 |
...
对应 DacFx `DacDeployOptions` / Schema Compare 选项，不做自研规则引擎。
...
路径：`%AppData%/FakeSqlCompare/projects.json`。密码用 **DPAPI（CurrentUser）** 加密，不进仓库、不进日志。启动时恢复上次填写的源/目标；「保存」更新当前项，「另存为」可起别名写入列表。无数据库迁移脚本。
...
| 步骤 | 可点内容 |
|---|---|
| 连接 | 源 / 目标；中间 **⇄ 互换**；测连列库；**保存 / 另存为（别名）/ 删除**；SQL 登录 PasswordBox |
| 对比中 | 测连源/目标 → DacFx 抽取对比 → 整理差异（可返回取消） |
| 差异工作台 | 真实差异列表；**互换源/目标**（互换后自动重对比）；**重新对比**；类型/状态芯片；左右定义 |
...
| 对比 | `Services/CompareService.cs`：`SchemaCompareDatabaseEndpoint` + `SchemaComparison.Compare`；顶层对象映射到列表；行 diff |
| 脚本 | 勾选后 `Result.ExcludeAll/Include` + `GenerateScript`（依赖顺序）；导出 `.sql` |
| 部署 | 成功后**自动重新对比**；生成脚本时 Include 子节点和依赖，避免勾了表却没更新列/约束 |
| UI | 顶栏徽章；库名下拉；工作台 **重新对比**；连接页 **保存 / 另存为别名 / 互换源目标** |
